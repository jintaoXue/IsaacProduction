...
def draw_metric_from_csv_res(ppo_loss, data_list, algo_dict, titles, x_lables, color_dict, log_x, y_tick_f, y_tick, smooth_alphas):
    ...
    for ax in fig.get_axes():
        ax.grid(True)
    # plt.tick_params(axis='both', labelsize=50)
    ax_inset = ax_1.inset_axes([0.55, 0.32, 0.38, 0.38])  # [左, 下, 宽, 高] 相对于主图
    draw_loss_ax(ax_inset, ppo_loss, titles[0], x_lables[0], log_x[0], y_tick_f[0], y_tick[0], smooth_alphas[0])
...
def draw_loss_ax(ax, ppo_loss, title, x_lable, log_x, y_tick_f, y_tick, alpha):
    '''loss curve plot'''
    color_dict = {"EPPO_actor": 'silver', "EPPO_critic": 'silver'}
    algo_dict = {"EPPO_actor":"ppo_discrete_2025-05-21_16-02-29"}
    draw_one_sub_pic(ax, ppo_loss[0], '', '', algo_dict, color_dict, log_x, y_tick_f, y_tick, alpha, '--')
    algo_dict = {"EPPO_critic":"ppo_discrete_2025-05-21_16-02-29"}
    draw_one_sub_pic(ax, ppo_loss[1], '', '', algo_dict, color_dict, log_x, y_tick_f, y_tick, alpha)
    ax.get_legend().remove()
    leg = ax.legend(ncol=1)
    # change the line width for the legend
    for line in leg.get_lines():
        line.set_linewidth(6.0)
    ax.legend(loc='center left', bbox_to_anchor=(0.05, 0.55), fontsize=15, frameon=True)

    return


def draw_one_sub_pic(ax, data, title, x_lable, algo_dict, color_dict, log_x, y_tick_f, y_tick, alpha, linestyle='-'):
    '''loss curve plot'''
    ...
    for name in algo_dict.values():
        ...
        ax.plot(x, smoothed_y, linestyle, color=color, label=label, ms=5, linewidth=2)
    # ax.legend(
    #     fontsize="x-large",
    #     handlelength=5.0)
        # handleheight=3)
...
if __name__ == '__main__':
    ### training metric
    t_loss = os.path.dirname(__file__) + "/t_loss.csv"
    ppo_critic_loss = os.path.dirname(__file__) + "/ppo_critic_loss.csv"
    ppo_actor_loss = os.path.dirname(__file__) + "/ppo_actor_loss.csv"
    t_return = os.path.dirname(__file__) + "/t_return.csv"
    t_len = os.path.dirname(__file__) + "/t_len.csv"
    t_succ = os.path.dirname(__file__) + "/t_success.csv"
    # t_buffer = os.path.dirname(__file__) + "/metric1" + "/t_buffer.csv"
    algo_dict = {"EPPO":"ppo_discrete_2025-05-21_16-02-29", 
    ...
    color_dict = {'EDQN1': 'orange', 'EDQN2': 'forestgreen', 'EBQ-G': 'dodgerblue', 'EBQ-N': 'palevioletred', 'EBQ-GN':'blueviolet', "EPPO": 'silver'}
    ...
    draw_metric_from_csv_res([ppo_actor_loss, ppo_critic_loss], data_list, algo_dict, titles, x_lables, color_dict, log_x, y_tick_f, y_tick, smooth_alphas)
