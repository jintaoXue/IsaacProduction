...
def draw_metric_from_csv_res(data_list, algo_dict, titles, x_lables, color_dict, log_x, y_tick_f, y_tick, smooth_alphas, save_name):
    ...
    for ax in fig.get_axes():
        ax.grid(True)
    # plt.tick_params(axis='both', labelsize=50)
    for i, ax in zip(range(0, len(data_list)), [ax_1, ax_2]):
...
if __name__ == '__main__':
    ### training metric
    # e_return = os.path.dirname(__file__) + "/e_return.csv"
    e_len = os.path.dirname(__file__) + "/e_len.csv"
    e_succ = os.path.dirname(__file__) + "/e_success.csv"
    # t_buffer = os.path.dirname(__file__) + "/metric1" + "/t_buffer.csv"
    algo_dict = {
                "No_spatial":"no_spatial_rainbowmini_2024-12-23_18-12-29",
                "Longest_path":"long_rainbowmini_2025-05-25_11-32-15", 
                "EBQ-N":"FactoryTaskAllocationMiC_2024-12-08_15-44-10", 
                 }
    color_dict = {'No_spatial': 'darkorchid', 'Longest_path': 'tomato', 'EBQ-N': 'palevioletred'}
    
    data_list = [e_len, e_succ]
    titles = ["Makespan", "Progress"]
    x_lables = ["Episode", "Episode"]
    log_x = [False, False]
    y_tick_f = [False, True]
    y_tick = [[], [0.8, 1.01]]
    ...
    draw_metric_from_csv_res(data_list, algo_dict, titles, x_lables, color_dict, log_x, y_tick_f, y_tick, smooth_alphas, 'low_level_metric')
