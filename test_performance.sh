#！/bin/bash
# 读取不同的学习率
load_dir="/FactoryTaskAllocationMiC2024-12-04_16-19-22/nn"
...
for filename in $files
do
    python omniisaacgymenvs/scripts/rlgames_train_v1.py task=FactoryTaskAllocationMiC train=FactoryTaskAllocationMiCRainbowmini headless=True wandb_activate=False test=True load_dir="$load_dir" load_name="$str$filename"
#    echo $filename >> filename.txt
#    echo -e >> filename.txt
done
# # 循环执行命令
